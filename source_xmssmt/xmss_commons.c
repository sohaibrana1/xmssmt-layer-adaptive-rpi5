                   ...
                   const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    uint32_t i;
    uint32_t height = 0;
    HBS_COMPONENT_BEGIN(HBS_COMPONENT_LTREE);
    ...
    HBS_COMPONENT_END(HBS_COMPONENT_LTREE);
}
                   ...
                   uint32_t ltree_addr[8], uint32_t ots_addr[8])
{
    ...
    HBS_COMPONENT_BEGIN(HBS_COMPONENT_LEAF_GENERATION);
    wots_pkgen(params, pk, sk_seed, pub_seed, ots_addr);

    l_tree(params, leaf, pk, pub_seed, ltree_addr);
    HBS_COMPONENT_END(HBS_COMPONENT_LEAF_GENERATION);
}
                          ...
                          const unsigned char *pk)
{
    ...
    unsigned int max_wots_sig_bytes = 0;
    unsigned int layer_scan;

    for (layer_scan = 0; layer_scan < params->d; layer_scan++) {
        if (params->wots_sig_bytes_layer[layer_scan]
                > max_wots_sig_bytes) {
            max_wots_sig_bytes =
                params->wots_sig_bytes_layer[layer_scan];
        }
    }

    if (max_wots_sig_bytes == 0) {
        return -1;
    }

    unsigned char wots_pk[max_wots_sig_bytes];
    ...
    *mlen = smlen - params->sig_bytes;
    ...
    for (i = 0; i < params->d; i++) {
        ...
        idx_leaf =
            (uint32_t)(
                idx
                & ((1ULL << layer_params.tree_height) - 1ULL)
            );

        idx = idx >> layer_params.tree_height;
        ...
        wots_pk_from_sig(&layer_params, wots_pk, sm,
                         root, pub_seed, ots_addr);
        sm += layer_params.wots_sig_bytes;

        /* Compute the leaf node using the WOTS public key. */
        set_ltree_addr(ltree_addr, idx_leaf);
        l_tree(&layer_params, leaf, wots_pk,
               pub_seed, ltree_addr);

        /* Compute the root node of this subtree. */
        compute_root(&layer_params, root, leaf,
                     idx_leaf, sm, pub_seed, node_addr);
